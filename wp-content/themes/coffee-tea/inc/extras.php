 ...
 * @return array
 */
function coffee_tea_body_classes( $classes ) {
...
add_filter( 'body_class', 'coffee_tea_body_classes' );
 ...
 */
 ...
 if ( ! function_exists( 'coffee_tea_breadcrumb_title' ) ) {
	function coffee_tea_breadcrumb_title() {
		...
		/* translators: %s: Daily Archives. */
			printf( __( 'Daily Archives: %s', 'coffee-tea' ), get_the_date() );
		
		elseif ( is_month() ) :
		/* translators: %s: Monthly Archives. */
			printf( __( 'Monthly Archives: %s', 'coffee-tea' ), (get_the_date( 'F Y' ) ));
		
		elseif ( is_year() ) :
		/* translators: %s: Yearly Archives. */
			printf( __( 'Yearly Archives: %s', 'coffee-tea' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		/* translators: %s: Category Archives. */
			printf( __( 'Category Archives: %s', 'coffee-tea' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		/* translators: %s: Tag Archives. */
			printf( __( 'Tag Archives: %s', 'coffee-tea' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'coffee-tea' ));
			
		elseif ( is_author() ) :
		/* translators: %s: Author. */
			printf( __( 'Author: %s', 'coffee-tea' ), (get_the_author( '', false ) ));		
...
function coffee_tea_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','coffee-tea'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','coffee-tea'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','coffee-tea'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active">'; // Tag before the current Breadcrumb
	$after 		= '</li>'; // Tag after the current Breadcrumb
	$seprator	= get_theme_mod('coffee_tea_breadcrumb_seprator','/');
	global $post;
	$homeLink = home_url();
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','coffee-tea').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','coffee-tea').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','coffee-tea').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','coffee-tea').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','coffee-tea'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','coffee-tea') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
function coffee_tea_default_styles() {
    // Begin Style
    $css = '<style>';

    $logo_width = get_theme_mod('coffee_tea_logo_width');
    $css .= '
...
add_action('wp_head', 'coffee_tea_default_styles', 999); // Use a priority to ensure it loads after other styles